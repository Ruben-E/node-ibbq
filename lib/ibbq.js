...
iBBQ.prototype.startMeasurements = function (callback) {
    this.enableRealTimeData().then(_ => {
        this.realTimeDataCharacteristic.on('data', data => {
            const probes = data.toString('utf8').length / 2
            const measurements = []

            for (let i = 0; i < data.length; i++) {
                if (i % 2 === 0) {
                    const probe = (i / 2) + 1;
                    let temperature = data.readIntLE(i, 2) / 10;
                    if (temperature === -1) {
                        temperature = null;
                    }

                    measurements.push(new Measurement(probe, temperature));
                }
            }
